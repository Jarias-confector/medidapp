...
export default function Photo() {
  ...
  async function pick(fromCamera: boolean) {
    ...
    setBusy(false);
    if (error || data?.error) { setMsg(data?.error ?? 'Falló el análisis. Intenta de nuevo.'); return; }
    if (!data.items?.length) { setMsg('No se reconoció comida en la foto. Registra manualmente.'); return; }
